...
uint64_t lsappend(const uint64_t kmer, const uint64_t c, const uint64_t k) { /* left shift and append */
    return ((kmer << 2) | c) & ((1UL << 2 * k) - 1);
}

uint64_t rsprepend(const uint64_t kmer, const uint64_t c, const uint64_t k) { /* right shift and prepend */
